...
/**
 ...
 [..]
   (#) Enable peripheral clock using the following functions
       PeripheralEnable(SPIx, true) for SPIx.
   ...
   (#) Program the Polarity, Phase, First Data, Baud Rate Prescaler, Slave
       ...
       (++) I2S PLL is configured using the functions
            ClockMultiply(ClockNodePLL2, ClockRatio(867.84 / 12.0));  // PLL2:  867.84 MHz
            ClockDivide(ClockNodePLL2I, ClockRatio(867.84 / 12.288)); // PLL2I: 12.288 MHz
            ClockSelect(ClockNodeI2S, ClockNodePLL2I);                // Select PLL2I as I2S Clock Source
            ClockEnable(ClockNodePLL2G, true);                        // Enalbe PLL2
            or
       (++) External clock source is configured using the function
            ClockSelect(ClockNodeI2S, ClockNodeI2SE); // Select I2S external clock source as I2S Clock Source
            and after setting correctly the define constant CONFIG_I2SE_VALE in the <target>_conf.h file.
 ...
 [..]
   (@) In I2S mode: if an external clock is used as source clock for the I2S,
       then the define CONFIG_I2SE_VALE in the <target>_conf.h should be enabled
...
void SPI_I2S_DeInit(SPI_TypeDef* SPIx)
{
    ...
    PeripheralReset(PeripheralFind(SPIx, PeripheralSPIStart, PeripheralSPIEnd));
}
...
void SPI_Init(SPI_TypeDef* SPIx, SPI_InitTypeDef* SPI_InitStruct)
{
    ...
    tmpreg |= (uint16_t)((uint32_t)SPI_InitStruct->SPI_Direction | SPI_InitStruct->SPI_Mode | SPI_InitStruct->SPI_DataSize | SPI_InitStruct->SPI_CPOL |
                         SPI_InitStruct->SPI_CPHA | SPI_InitStruct->SPI_NSS | SPI_InitStruct->SPI_BaudRatePrescaler | SPI_InitStruct->SPI_FirstBit);
    /* Write to SPIx CR1 */
    SPIx->CR1 = tmpreg;
 ...
 *         CONFIG_I2SE_VALE in the <target>_conf.h should be enabled and set to the value of
 *         the source clock frequency (in Hz).
 *
 * @retval None
 */
void I2S_Init(SPI_TypeDef* SPIx, I2S_InitTypeDef* I2S_InitStruct)
{
    ...
    tmpreg |= (uint16_t)((uint16_t)SPI_I2SCFGR_I2SMOD |
                         (uint16_t)(I2S_InitStruct->I2S_Mode | (uint16_t)(I2S_InitStruct->I2S_Standard |
...
void I2S_FullDuplexConfig(SPI_TypeDef* I2Sxext, I2S_InitTypeDef* I2S_InitStruct)
{
    ...
    /* Configure the I2S with the SPI_InitStruct values */
    tmpreg |=
        (uint16_t)((uint16_t)SPI_I2SCFGR_I2SMOD | (uint16_t)(tmp | (uint16_t)(I2S_InitStruct->I2S_Standard | (uint16_t)(I2S_InitStruct->I2S_DataFormat |
...
@verbatim
 ...
 [..]
   ...
   (@) When the device is configured as slave and the NSS hardware mode is
...
void SPI_TxFIFOCmd(SPI_TypeDef* SPIx, FunctionalState NewState)
{
	...
	if (NewState != DISABLE)
    {
        SPIx->CR3 |= SPI_CR3_TXFIFOEN;
    }
    else
    {
        SPIx->CR3 &= (uint32_t)~SPI_CR3_TXFIFOEN;
    }
}
...
void SPI_RxFIFOCmd(SPI_TypeDef* SPIx, FunctionalState NewState)
{
	...
	if (NewState != DISABLE)
    {
        SPIx->CR3 |= SPI_CR3_RXFIFOEN;
    }
    else
    {
        SPIx->CR3 &= (uint32_t)~SPI_CR3_RXFIFOEN;
    }
}
...
void SPI_FIFOITConfig(SPI_TypeDef* SPIx, uint32_t SPI_FIFO_IT, FunctionalState NewState)
{
	/* Check the parameters */
    assert_param(IS_SPI_ALL_PERIPH_EXT(SPIx));
	assert_param(IS_SPI_FIFO_IT(SPI_FIFO_IT));

	if (NewState != DISABLE)
    {
        SPIx->CR3 |= SPI_FIFO_IT;
    }
    else
    {
        SPIx->CR3 &= (uint32_t)~SPI_FIFO_IT;
    }
}
...
void SPI_TxFIFOThresholdConfig(SPI_TypeDef* SPIx, uint8_t Threshold)
{
	/* Check the parameters */
    assert_param(IS_SPI_ALL_PERIPH_EXT(SPIx));
	assert_param(IS_SPI_TXFIFO_THRESHOLD(Threshold));
	SPIx->CR3 |= (Threshold << SPI_TXFIFO_POS);
}
...
void SPI_RxFIFOThresholdConfig(SPI_TypeDef* SPIx, uint8_t Threshold)
{
    ...
    assert_param(IS_SPI_ALL_PERIPH_EXT(SPIx));
	assert_param(IS_SPI_TXFIFO_THRESHOLD(Threshold));
	SPIx->CR3 |= (Threshold << SPI_RXFIFO_POS);
}
...
FlagStatus SPI_FIFO_GetFlagStatus(SPI_TypeDef* SPIx, uint32_t SPI_FIFO_FLAG){
	...
	/* Check the parameters */
    assert_param(IS_SPI_ALL_PERIPH_EXT(SPIx));
	assert_param(IS_SPI_FIFO_FLAG(SPI_FIFO_FLAG));

    /* Check the status of the specified SPI FIFO flag */
    if ((SPIx->SR2 & SPI_FIFO_FLAG) != (uint16_t)RESET)
    {
        /* SPI_I2S_FLAG is set */
        bitstatus = SET;
    }
...
void SPI_AsyncModeCmd(SPI_TypeDef* SPIx, FunctionalState NewState){
	...
	if (NewState != DISABLE)
    {
        SPIx->EXTR |= SPI_EXTR_ASYNCEN;
    }
    else
    {
        SPIx->EXTR &= (uint32_t)~SPI_EXTR_ASYNCEN;
    }
}
...
void SPI_AsyncSampleDelayCmd(SPI_TypeDef* SPIx, FunctionalState NewState){
	...
	if (NewState != DISABLE)
    {
        SPIx->EXTR |= SPI_EXTR_ASYNC_SAMPLE_DELAY;
    }
    else
    {
        SPIx->EXTR &= (uint32_t)~SPI_EXTR_ASYNC_SAMPLE_DELAY;
    }
}
...
void SPI_ReceiveSampleDelayCmd(SPI_TypeDef* SPIx, FunctionalState NewState){
	...
	if (NewState != DISABLE)
    {
        SPIx->EXTR |= SPI_EXTR_RX_SAMPLE_DEALY;
    }
    else
    {
        SPIx->EXTR &= (uint32_t)~SPI_EXTR_RX_SAMPLE_DEALY;
    }
}
...
void SPI_TXFIFOClear(SPI_TypeDef* SPIx){
	assert_param(IS_SPI_ALL_PERIPH_EXT(SPIx));
	SPIx->FIFOCLRR |= SPI_TXFIFO_CLEAR;
	SPIx->FIFOCLRR &= ~SPI_TXFIFO_CLEAR;
}

void SPI_RXFIFOClear(SPI_TypeDef* SPIx){
	assert_param(IS_SPI_ALL_PERIPH_EXT(SPIx));
	SPIx->FIFOCLRR |= SPI_RXFIFO_CLEAR;
	SPIx->FIFOCLRR &= ~SPI_RXFIFO_CLEAR;
}
