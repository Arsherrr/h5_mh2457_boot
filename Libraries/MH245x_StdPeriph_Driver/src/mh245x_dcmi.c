...
{
    PeripheralReset(PeripheralDCMI);
}
...
void DCMI_Init(DCMI_InitTypeDef* DCMI_InitStruct)
{
    ...
    assert_param(IS_DCMI_DMAREQUEST_SIZE(DCMI_InitStruct->DCMI_DMARequestSize));
#endif
    ...
    reg |= DCMI_InitStruct->DCMI_CaptureMode |      //
    ...
    DCMI->DMACR = DCMI_InitStruct->DCMI_DMARequestSize << 29;
#elif FEATURE_DCMI_CR_REQSIZE
    reg &= ~DCMI_CR_DMAS;
    reg |= DCMI_InitStruct->DCMI_DMARequestSize << 29;
#endif
...
#if (FEATURE_DCMI_CR_REQSIZE || FEATURE_DCMI_FIFOSR_DMACR)
    DCMI_InitStruct->DCMI_DMARequestSize = 4;
#endif
}
...
{
    DCMI->ESCR = (uint32_t)(DCMI_CodesInitStruct->DCMI_FrameStartCode | ((uint32_t)DCMI_CodesInitStruct->DCMI_LineStartCode << 8) |
                            ((uint32_t)DCMI_CodesInitStruct->DCMI_LineEndCode << 16) | ((uint32_t)DCMI_CodesInitStruct->DCMI_FrameEndCode << 24));
}
...
{
    /* Clear the interrupt pending Bit by writing in the ICR register 1 in the corresponding pending Bit position */
    DCMI->ICR = DCMI_IT;
}
