 ...
 * @param PHY_Descrip: external PHY description
...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, PHY_DescripDef* PHY_Descrip)
{
    ...
    if (!(ETH_WritePHYRegister(PHY_Descrip->Phy_Address, PHY_BCR, PHY_Reset)))
    {
    ...
    if (ETH_InitStruct->ETH_AutoNegotiation != ETH_AutoNegotiation_Disable)
    {
        ...
        do
        {
            phy_bsr = ETH_ReadPHYRegister(PHY_Descrip->Phy_Address, PHY_BSR);
            timeout++;
        }
        while (!(phy_bsr & PHY_Linked_Status) && (timeout < PHY_READ_TO));	//new added timeout judge
        ...
        if (!(ETH_WritePHYRegister(PHY_Descrip->Phy_Address, PHY_BCR, PHY_AutoNegotiation)))
        {
            /* Return ERROR in case of write timeout */
            err = ETH_ERROR;
        }
        ...
        while (!(ETH_ReadPHYRegister(PHY_Descrip->Phy_Address, PHY_BSR) & PHY_AutoNego_Complete) && (timeout < (uint32_t)PHY_READ_TO));
        ...
        RegValue = ETH_ReadPHYRegister(PHY_Descrip->Phy_Address, PHY_Descrip->Phy_SR_Offset);
        /* Configure the MAC with the Duplex Mode fixed by the auto-negotiation process */
        if (RegValue & PHY_Descrip->Phy_Duplex_Mask)
        {
        ...
        if (RegValue & PHY_Descrip->Phy_Speed_Mask)
        {
    ...
    {
        if (!ETH_WritePHYRegister(PHY_Descrip->Phy_Address, PHY_BCR, ((uint16_t)(ETH_InitStruct->ETH_Mode >> 3) | (uint16_t)(ETH_InitStruct->ETH_Speed >> 1))))
        {
            /* Return ERROR in case of write timeout */
            err = ETH_ERROR;
        }
...
error:
    ...
    tmpreg |=
        (uint32_t)(ETH_InitStruct->ETH_DropTCPIPChecksumErrorFrame | ETH_InitStruct->ETH_ReceiveStoreForward | ETH_InitStruct->ETH_FlushReceivedFrame |
 ...
 * @param  ETH_MediaInterface: specifies the Media Interface mode.
 *          This parameter can be one of the following values:
...
void ETH_MediaInterfaceConfig(uint32_t ETH_MediaInterface) {
    assert_param(IS_ETH_MEDIA_INTERFACE(ETH_MediaInterface));
    /* Configure MII_RMII selection bit */
    SYSCFG->PMC = (SYSCFG->PMC & (~SYSCFG_PMC_MII_RMII_SEL)) | ETH_MediaInterface;
}
