 ...
 * @param  USARTx: where x can be 1, 2, 3 ... to select the USART or UART peripheral.
 * @retval None
 */
void USART_DeInit(USART_TypeDef* USARTx)
{
    PeripheralReset(PeripheralFind(USARTx, PeripheralUARTStart, PeripheralUARTEnd));
}
...
void USART_Init(USART_TypeDef* USARTx, USART_InitTypeDef* USART_InitStruct)
{
    ...
    apbclock = ClockGet(ClockNodeAPB);
    ...
    if (USART_InitStruct->USART_RxFIFO)
    {
        USARTx->CR4 &= ~BIT1; // Enable rx fifo
    }
    else
    {
        USARTx->CR4 |= BIT1;
...
void USART_StructInit(USART_InitTypeDef* USART_InitStruct)
{
    ...
    USART_InitStruct->USART_RxFIFO = USART_RxFIFO_Disable;
}
...
void USART_ClockInit(USART_TypeDef* USARTx, USART_ClockInitTypeDef* USART_ClockInitStruct)
{
    ...
    tmpreg |= (uint32_t)USART_ClockInitStruct->USART_Clock | USART_ClockInitStruct->USART_CPOL | USART_ClockInitStruct->USART_CPHA |
 ...
 * @note   The function is used for IrDA mode with UART5.
 * @retval None
 */
 ...
 * @param  USARTx: where x can be 1, 2, 3 or 6 to select the USART or UART peripheral.
 ...
 *            @arg USART_IT_RFAF: Rx FIFO Almost Full interrupt
...
void USART_ITConfig(USART_TypeDef* USARTx, uint16_t USART_IT, FunctionalState NewState)
{
    ...
    /* The CTS interrupt is not available for UART4 */
    ...
    else /* The IT is in CR4 register */
    {
        usartxbase += 0x1C;
    }
 ...
 *            @arg USART_FLAG_RFAF: Rx FIFO Almost Full flag
...
FlagStatus USART_GetFlagStatus(USART_TypeDef* USARTx, uint16_t USART_FLAG)
{
    ...
    /* The CTS flag is not available for UART4 */
    if (USART_FLAG == USART_FLAG_CTS)
    {
        assert_param(IS_USART_PERIPH(USARTx));
    }

    __IO uint16_t sr = USARTx->SR;

    sr |= (USARTx->CR4 & BIT5) << 5;
    if ((sr & USART_FLAG) != (uint16_t)RESET)
    {
        bitstatus = SET;
    }
    else
 ...
 *            @arg USART_IT_PE:   Parity Error interrupt
 *            @arg USART_IT_RFAF: Rx FIFO almost full
...
ITStatus USART_GetITStatus(USART_TypeDef* USARTx, uint16_t USART_IT)
{
    ...
    bitpos &= ((USARTx->CR4 & BIT5) << 5) | USARTx->SR;
