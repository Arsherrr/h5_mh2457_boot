...
static void indev_keypad_proc(lv_indev_t * i, lv_indev_data_t * data)
{
    ...
    /*Pressing*/
        ...
        if(i->long_pr_sent == 0 && lv_tick_elaps(i->pr_timestamp) > i->long_press_time) {
            i->long_pr_sent = 1;
            if(data->key == LV_KEY_ENTER || data->key == LV_KEY_UP || data->key == LV_KEY_DOWN) {
    ...
    /*Release happened*/
        ...
        data->key = prev_key;
        if(data->key == LV_KEY_ENTER || data->key == LV_KEY_UP || data->key == LV_KEY_DOWN || data->key == LV_KEY_BACKSPACE) {
