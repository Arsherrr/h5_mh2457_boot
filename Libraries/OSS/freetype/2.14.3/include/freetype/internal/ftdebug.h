...
static inline void rtt_ft_log_wrapper(const char* format, ...)
{
    va_list args;
    va_start(args, format);

    SEGGER_RTT_printf(0, format, &args);
    va_end(args);
}

#define FT_LOG_RTT( level, varformat )                                         \
          do                                                               \
          {                                                                \
              rtt_ft_log_wrapper varformat; \
            if ( ft_trace_levels[FT_TRACE_COMP( FT_COMPONENT )] >= level ) \
                ;                                      \
          } while ( 0 )
...
#define FT_TRACE( level, varformat )  FT_LOG_RTT( level, varformat )
...
#define FT_TRACE1( varformat )  FT_TRACE( 0, varformat )
