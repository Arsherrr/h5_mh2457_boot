...
static void Init(InterfaceCCIDStruct* self) {
    ...
    util_assert(self->ClassDescriptor.bMaxSlotIndex < CCID_MAX_SUPPORTED_SLOT);

    self->TxEPConfig.Buffer = self->ResponseBuffer;
    self->RxEPConfig.Buffer = self->CommandBuffer;

    for (int i = 0; i < CCID_MAX_SUPPORTED_SLOT; i++) {
        self->Slot[i].State       = 0;
        self->Slot[i].bmICCStatus = CCIDCardNotPresent;
    }

    uint8_t epCount = 2;
#ifdef CCID_SUPPORT_NOTIFICATION
    self->IntEPConfig.Buffer = self->ResponseBuffer;
    if (self->IntEPConfig.Index) {
        epCount += 1;
    }
#endif // CCID_SUPPORT_NOTIFICATION
    self->base.EPCount = epCount;
}
...
static bool IEPCallback(InterfaceCCIDStruct* self, uint32_t* buffer, uint32_t bytesWritten) {
    switch (self->State) {
        case CCIDStateSendResponse:
            if (bytesWritten > 0 && bytesWritten % self->base.Device->HAL->EP[self->TxEPConfig.Index][EPDirectIn].MaxPacket == 0) {
...
#ifdef CCID_SUPPORT_NOTIFICATION
static bool IntIEPCallback(InterfaceCCIDStruct* self, uint32_t* buffer, uint32_t bytesWritten) {
    for (int i = 0; i <= self->ClassDescriptor.bMaxSlotIndex; i++) {
        self->Slot[i].IsChanging = false;
    }
    return true;
}

static inline bool Notify(InterfaceCCIDStruct* self, uint8_t bSlot, bool isPresent) {
    USBDeviceStruct* device = self->base.Device;

    if (self->Slot[bSlot].IsPresent == isPresent) {
        return false;
    }
    ...
    for (int i = 0; i <= self->ClassDescriptor.bMaxSlotIndex; i++) {
        if (self->Slot[i].IsChanging)
            return false;
    }

    self->Slot[bSlot].IsPresent   = isPresent;
    self->Slot[bSlot].IsChanging  = true;
    self->Slot[bSlot].IsActive    = false;
    self->Slot[bSlot].bmICCStatus = isPresent ? CCIDCardPresentInactive : CCIDCardNotPresent;

    self->Notification = (CCIDNotificationStruct) {
        .bMessageType = RDR_TO_PC_NOTIFYSLOTCHANGE,
    };

    for (int i = 0; i <= self->ClassDescriptor.bMaxSlotIndex; i++) {
        self->Notification.bmSlotICCState[i / 4] |= (self->Slot[bSlot].State & 0x3) << (i % 4 * 2);
    }

    USBHALStruct* hal = device->HAL;
    return hal->EndpointStartWrite(hal, self->IntEPConfig.Index, (uint32_t*)&self->Notification, sizeof(CCIDNotificationStruct));
}
#endif // CCID_SUPPORT_NOTIFICATION
...
static bool CommandCheck(InterfaceCCIDStruct* self, uint32_t checkItems) {
    ...
    if (checkItems & (CCIDCheckCardPresent | CCIDCheckActive)) {
        if (self->Slot[self->Command->bSlot].bmICCStatus) {
            self->Response->bmICCStatus = self->Slot[self->Command->bSlot].bmICCStatus;
            return CommandResult(self, self->Slot[self->Command->bSlot].IsActive ? CCID_ERROR_HW_ERROR : CCID_ERROR_ICC_MUTE);
        }
    }
...
static void PC_to_RDR_IccPowerOn(InterfaceCCIDStruct* self, CCIDCommandStruct* command, CCIDResponseStruct* response) {
    ...
    */
    ...
    self->Slot[command->bSlot].IsActive    = true;
    self->Slot[command->bSlot].bmICCStatus = CCIDCardPresentActive;
}
...
static void PC_to_RDR_IccPowerOff(InterfaceCCIDStruct* self, CCIDCommandStruct* command, CCIDResponseStruct* response) {
    ...
    self->Slot[command->bSlot].IsActive    = false;
    self->Slot[command->bSlot].bmICCStatus = CCIDCardPresentInactive;
}
...
bool InterfaceCCIDConstractor(InterfaceCCIDStruct* self) {
    memset(self, 0, sizeof(InterfaceCCIDStruct));
    ...
    self->IntEPConfig = (EPConfigStruct) {
        ...
        .MaxPacket  = 8,
        .MaxBuffer  = sizeof(CCIDNotificationStruct),
        .Buffer     = NULL,
        .Callback   = (EPCallbackType)IntIEPCallback,
        .Interval   = 4,
        .HSInterval = 6,
    };
    ...
    USBInterfaceConstractor((USBInterfaceStruct*)self, CCID_CLASS, 0x00, 0x00, 2, self->InterfaceEPConfig);
