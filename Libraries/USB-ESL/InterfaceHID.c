...
bool InterfaceHIDConstractor(InterfaceHIDStruct* self) {
    memset(self, 0, sizeof(InterfaceHIDStruct));

    self->base.Init = (USBInterfaceInitType)Init;

    self->base.InterfaceExtendDescriptorSize = HID_DESCRIPTOR_SIZE;
    self->base.InterfaceExtendDescriptor     = (uint8_t*)self->HIDDescriptorData;
    ...
    USBInterfaceConstractor((USBInterfaceStruct*)self, HID_CLASS, HID_SUBCLASS_NONE, HID_PROTOCOL_NONE, 2, self->InterfaceEPConfig);
    return true;
}
