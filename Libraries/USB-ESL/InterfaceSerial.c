...
static void Init(InterfaceSerialStruct* self) {
    ...
    self->IsConnected = self->TxEPConfig.Timeout ? false : true;
...
void StartProbe(InterfaceSerialStruct* self) {
    ...
    if (hal->EndpointStartProbe(hal, self->TxEPConfig.Index)) {
        self->ProbeTimeout = self->TxEPConfig.Timeout * 10;
    }
}
...
static bool AsyncTask(InterfaceSerialStruct* self) {
    ...
    if (self->ProbeTimeout) {
        if (!(--self->ProbeTimeout)) {
            // Connect Probe failed, disconnect
            if (self->IsConnected) {
    ...
    if (self->IsConnected && (SERIAL_TX_QUEUE_LENGTH - self->SendBlockCount >= 2) && circular->StartPop(circular, circular->Length, true, true)) {
...
static bool Sent(InterfaceSerialStruct* self, uint32_t* buffer, uint32_t bytesWritten) {
    ...
    if (bytesWritten % hal->EP[self->TxEPConfig.Index][EPDirectIn].MaxPacket == 0) {
...
bool InterfaceSerialConstractor(InterfaceSerialStruct* self) {
    memset(self, 0, sizeof(InterfaceSerialStruct));
    ...
    USBInterfaceConstractor((USBInterfaceStruct*)self, 0x0A, 0x00, 0x00, 2, self->InterfaceEPConfig);
    return true;
}
