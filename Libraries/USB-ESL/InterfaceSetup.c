        ...
        */
        ...
        if (transfer->Packet.wLength > 0) {
            if (transfer->Direction == DEVICE_TO_HOST) {
                ...
                hal->EndpointStartRead(hal, self->RxEPConfig.Index, (uint32_t*)transfer->Buffer, transfer->Length);
            }
        }
        else {
        ...
        if (transfer->Length) {
            // Meano 20230413 Out Process should be done after read all data
            return false;
        }
...
bool InterfaceSetupConstractor(InterfaceSetupStruct* self) {
    memset(self, 0, sizeof(InterfaceSetupStruct));
    ...
    self->TxEPConfig = (EPConfigStruct) {
        ...
        .MaxPacket = 64,
        .MaxBuffer = 64,
        .Buffer    = self->SetupBufferIn,
        .Callback  = (EPCallbackType)SetupCallbackIn,
    };
    ...
    self->RxEPConfig = (EPConfigStruct) {
        ...
        .MaxPacket = 64,
        .MaxBuffer = 64,
        .Buffer    = self->SetupBufferOut,
        .Callback  = (EPCallbackType)SetupCallbackOut,
    };

    USBInterfaceConstractor((USBInterfaceStruct*)self, 0, 0, 0, 2, self->InterfaceEPConfig);
