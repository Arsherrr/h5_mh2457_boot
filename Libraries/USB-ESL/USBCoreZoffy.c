...
#if FEATURE_USBCORE_ZOFFY_DYNAMIC_DMA_MODE
#define IsDMAEnabled() self->IsDMAEnabled
#else
#define IsDMAEnabled() FEATURE_USBCORE_ZOFFY_SUPPORT_DMA
#endif

#if FEATURE_USBCORE_ZOFFY_DYNAMIC_FIFO_MODE
#define IsDedicatedTFifo() self->Zoffy.IsDedicatedTFifo
#else
#define IsDedicatedTFifo() FEATURE_USBCORE_ZOFFY_SUPPORT_DEDICATED_FIFO
#endif
...
static bool Init(USBHALStruct* self) {
    ...
    if (self->IsDeviceEnabled != self->IsHostEnabled) {
        cfg |= self->IsDeviceEnabled ? BIT(30) : BIT(29);
    }
    ...
    if (IsDMAEnabled()) {
        usb->GREGS.GAHBCFG |= //
            BIT(23) |         // The remaining data in the transfer is sent using Single burst size.
            // BIT22 |    // All DMA Write Transactions   [22]
            // BIT21 |    // Remote Memory Support        [21]
            // BIT8 |     // Periodic TxFEmpLvl           [8] Host?
            USB_GAHBCFG_TXFELVL |         // NPeriodic TxFEmpLvl          [7]
            USB_GAHBCFG_DMAEN |           // DMA Enable                   [5]
            7 << USB_GAHBCFG_HBSTLEN_Pos; // DMA Burst Length             [4:1]
...
static void Reset(USBHALStruct* self) {
    ...
    // Clear registers of endpoints
    for (int i = 0; i < self->DeviceEndpointCount; i++) {
        usb->OEPREGS[i].DOEPTSIZ = 0;
        usb->OEPREGS[i].DOEPCTL  = USB_DOEPCTL_SNAK; // SNAK
        if (usb->OEPREGS[i].DOEPCTL & USB_DOEPCTL_EPENA) {
            usb->OEPREGS[i].DOEPCTL = USB_DOEPCTL_EPDIS;
        }
        usb->IEPREGS[i].DIEPTSIZ = 0;
        usb->IEPREGS[i].DIEPCTL  = USB_DIEPCTL_SNAK; // SNAK
        if (usb->IEPREGS[i].DIEPCTL & USB_DIEPCTL_EPENA) {
            usb->IEPREGS[i].DIEPCTL = USB_DIEPCTL_EPDIS;
        }
    }
    ...
    // OUT EP Msk
    usb->DREGS.DOEPMSK = USB_DOEPINT_STATUS |   // Status mask
                         USB_DOEPINT_SETUP |    // Setup mask
    ...
    if (IsDedicatedTFifo()) {
        ...
        // Init Shared TFIFO
        usb->GREGS.GINTMSK &= ~USB_GINTMSK_NPTXFEM;
        self->Zoffy.IsTxFifoBusy  = false;
        self->Zoffy.IEPProcessing = 0;
    }

    self->IEPPendingMask = 0;
    self->OEPPendingMask = 0;

    self->OEPRetryMask       = 0;
    self->Zoffy.OEPSplitMask = 0;

    // Init RX Fifo Size
    usb->GREGS.GRXFSIZ = self->Zoffy.MaxFifoSize;

    self->Zoffy.MinRxFifoSize = 0;

    // Clear Address
    usb->DREGS.DCFG &= ~(USB_DCFG_DAD_Msk);

    // Flush RX fifo
    usb->GREGS.GRSTCTL |= BIT(4);
    while (usb->GREGS.GRSTCTL & BIT(4)) {}
...
static void StartDevice(USBHALStruct* self, USBDeviceStruct* device) {
    self->Device        = device;
    USBZoffyStruct* usb = self->Zoffy.USB;
    usb->DREGS.DCTL     = USB_DCTL_IGNRFRMNUM;
...
static bool ActiveEndpoint(USBHALStruct* self, uint8_t epIndex, EPDirect epDirect) {
    ...
    // limit the max packet size for Full-Speed
    ep->MaxPacket = epConfig->MaxPacket;
    if (self->Speed != USBSpeedHigh)
        ep->MaxPacket = MIN(ep->MaxPacket, (epConfig->Type != EPTypeISO) ? 64 : 1020);

    uint32_t epReg = USB_DIEPCTL_SD0PID |                       // Set the EP specific control settings (SD0PID)
                     USB_DIEPCTL_SNAK |                         // SNAK
                     (epConfig->Type << USB_DIEPCTL_TYPE_Pos) | // Endpoint type
                     USB_DIEPCTL_ACTIVE |                       // Active endpoint
                     (epIndex ? ep->MaxPacket : 0);             // Single Packet Max Transfer Size

    ep->Status = EPCompleted;
    if (epDirect == EPDirectIn) { // In Endpoint
        // TxFIFO allocation
        uint16_t txFifoSize    = (ep->MaxPacket * (1 + epConfig->AdditionalBuffer)) >> 2;
        uint16_t txFifoAddress = usb->GREGS.GRXFSIZ;

        if (IsDedicatedTFifo()) {
            if (txFifoAddress < txFifoSize + self->Zoffy.MinRxFifoSize)
                return false;

            txFifoAddress -= txFifoSize;

            volatile uint32_t* txFifoReg = epIndex == 0 ? &usb->GREGS.GNPTXFSIZ : &usb->GREGS.DIEPTXF[self->Zoffy.TxFifoIndex - 1];

            // Set up the Tx FIFO
            *txFifoReg = (txFifoSize << 16) | txFifoAddress;

            // Flush TX fifo
            usb->GREGS.GRSTCTL = (usb->GREGS.GRSTCTL & ~BITS(10, 6)) | (self->Zoffy.TxFifoIndex << 6) | BIT(5);
            while (usb->GREGS.GRSTCTL & BIT(5)) {}

            epReg |= self->Zoffy.TxFifoIndex << 22; // TxFIFO index
            self->Zoffy.TxFifoIndex += 1;

            // Reduce Rx FIFO
            usb->GREGS.GRXFSIZ = txFifoAddress;
        }
        else if (txFifoSize > (self->Zoffy.MaxFifoSize - txFifoAddress)) {
            if (self->Zoffy.MaxFifoSize < txFifoSize + self->Zoffy.MinRxFifoSize)
                return false;

            txFifoAddress = self->Zoffy.MaxFifoSize - txFifoSize;

            usb->GREGS.GNPTXFSIZ = (txFifoSize << 16) | txFifoAddress;
            usb->GREGS.GRXFSIZ   = txFifoAddress;
        }
        ...
        /*
            MinRxFifoSize = (5 * number of control endpoints + 8) \
                + ((largest USB packet used / 4) + 1 for status information) * (1 + additional buffer count) \
                + (2 * number of OUT endpoints) + 1 for Global NAK
        */
        uint16_t epRxFifoSize = ((ep->MaxPacket >> 2) + 1) * (1 + epConfig->AdditionalBuffer);

        self->Zoffy.MinRxFifoSize = MAX(14 + epRxFifoSize + (2 * self->DeviceEndpointCount), self->Zoffy.MinRxFifoSize);
        if (usb->GREGS.GRXFSIZ < self->Zoffy.MinRxFifoSize)
            return false;

        // Enable Out EP split package when `epConfig->Timeout` valid
        if (epConfig->Timeout && epConfig->MaxBuffer > ep->MaxPacket) {
...
static void EndpointStall(USBHALStruct* self, uint8_t epIndex, EPDirect epDirect, bool isEnable) {
    ...
    if (isEnable) {
        ...
        if (epDirect == EPDirectOut) {
            ...
            usb->IEPREGS[epIndex].DIEPCTL |= USB_DIEPCTL_SD0PID;
            usb->IEPREGS[epIndex].DIEPCTL &= ~USB_DIEPCTL_STALL; // Out Stall
        }

        if (ep->Status == EPStalled)
            ep->Status = EPCompleted;
...
static bool EndpointStartRead(USBHALStruct* self, uint8_t epIndex, uint32_t* epBuffer, uint32_t bytesToRead) {
    ...
    ep->Buffer         = epBuffer;
    ep->TransferSize   = bytesToRead;
    ep->TransferOffset = 0;
    ep->PendingTick    = 0;
    ep->Status         = EPPending;

    if (ep->Config->Type == EPTypeControl) {
        bytesToRead = BIT(19) | MIN(bytesToRead, ep->MaxPacket);
    }
    else {
        bytesToRead |= MAX((bytesToRead + ep->MaxPacket - 1) / ep->MaxPacket, 1) << 19;
    }
...
static bool EndpointStartWrite(USBHALStruct* self, uint8_t epIndex, uint32_t* epBuffer, uint32_t bytesToWrite) {
    ...
    if (IsDMAEnabled()) {
        ...
        if (bytesToWrite && ((uint32_t)epBuffer % 4 || (uint32_t)epBuffer < 0x20000000)) {
    ...
    uint32_t epSizeReg = MAX((bytesToWrite + ep->MaxPacket - 1) / ep->MaxPacket, 1) << 19 | // multi packet
                         bytesToWrite;                                                      // Size of packet

    if (ep->Config->AdditionalPacket) {
        epSizeReg |= (epSizeReg & BITS(20, 19)) << 10;
    }

    usb->IEPREGS[epIndex].DIEPTSIZ = epSizeReg;

    /*
        TODO: When using Shared FIFO operation in Slave mode, when transmitting a zero-length data packet in Slave mode, check the GNPTXSTS register to ensure
            that at least one space is available in the Non-periodic Request Queue (NPTxQSpcAvail).
    */
    ...
    if (!ep->TransferSize) // zero-length transfer not need a processing
        return true;
...
static inline void EndpointProcessWrite(USBHALStruct* self, uint8_t epIndex) {
    ...
    uint16_t txMaxFifo = (IsDedicatedTFifo() ? usb->IEPREGS[epIndex].DTXFSTS : usb->GREGS.GNPTXSTS & BITS(15, 0)) << 2;

    uint32_t  bytesToWrite = MIN((ep->TransferSize - ep->TransferOffset), txMaxFifo);
    uint32_t* bufferPoint  = ep->Buffer + (ep->TransferOffset >> 2);
    ...
    if (ep->TransferOffset == ep->TransferSize) {
        self->IEPPendingMask &= ~BIT(epIndex);
        ...
        if (IsDedicatedTFifo())
            ...
            if (!self->IEPPendingMask)
                usb->GREGS.GINTMSK &= ~USB_GINTMSK_NPTXFEM; // Unmask TxFifo empty
            self->Zoffy.IsTxFifoBusy = false;
        }
    }
...
static void TickTask(USBHALStruct* self) {
    ...
    while (oEPPendingMask >>= 1) {
        ...
        if (readSize % ep->Config->MaxBuffer / ep->MaxPacket == 0)
            continue;
...
static inline void DeviceInterrupt(USBHALStruct* self, USBZoffyStruct* usb, uint32_t gintsts) {
    ...
    if (gintsts & USB_GINTSTS_OEPINT) { // [O] endpoint interrupt [R]
        ...
        while (epIntMask) {
            if (epIntMask & 1) {
                ...
                if (epIndex == 0) {
                    if (outEpInt & USB_DOEPINT_SETUP_PACKET) { // Setup packet received
                        if (outEpInt & USB_DOEPINT_COMPLETED) {
                            outEpInt &= ~USB_DOEPINT_COMPLETED;
                            usb->OEPREGS[epIndex].DOEPINT = USB_DOEPINT_COMPLETED;
                        }
                        usb->OEPREGS[epIndex].DOEPINT = USB_DOEPINT_SETUP_PACKET;
                    }

                    if (outEpInt & USB_DOEPINT_SETUP) {
                        usb->OEPREGS[epIndex].DOEPINT = USB_DOEPINT_SETUP;
                        ep->TransferSize |= USB_TRANSFER_SETUP;
                        EndpointEndRead(self, epIndex);
                    }

                    if (outEpInt & USB_DOEPINT_STATUS) {
                        usb->OEPREGS[epIndex].DOEPINT = USB_DOEPINT_STATUS;
                    }

                    if (outEpInt & USB_DOEPINT_COMPLETED) {
                        uint16_t bytesRemaining = ep->TransferSize - ep->TransferOffset;
                        if (IsDMAEnabled()) {
                            if (bytesRemaining <= ep->MaxPacket) {
                                bytesRemaining = 0;
                            }
                            else {
                                ep->TransferOffset += ep->MaxPacket;
                                bytesRemaining -= ep->MaxPacket;
                            }
                        }
                        if (bytesRemaining) {
                            usb->OEPREGS[0].DOEPINT  = USB_DOEPINT_COMPLETED;
                            usb->OEPREGS[0].DOEPTSIZ = BIT(19) | MIN(bytesRemaining, ep->MaxPacket);
                            usb->OEPREGS[0].DOEPCTL |= USB_DOEPCTL_EPENA | USB_DOEPCTL_CNAK;
                            break;
                        }
                    }
                }
    ...
    if (!IsDMAEnabled()) { // [O] RX FIFO not empty [R]
        while (usb->GREGS.GINTSTS & USB_GINTSTS_RXFLVL) {
            volatile uint32_t epRxStatus = usb->GREGS.GRXSTSR;

            uint8_t  type    = (epRxStatus >> 17) & 0xF;
            uint8_t  epIndex = epRxStatus & 0xF;
            uint16_t rxSize  = (epRxStatus >> 4) & 0x7FF;
    ...
    if (!IsDedicatedTFifo() && self->IEPPendingMask && (gintsts & USB_GINTSTS_NPTXFE)) {
        if (!self->Zoffy.IsTxFifoBusy) {
            uint32_t dtknqr = usb->DREGS.DTKNQR1;
            if (dtknqr & BIT(7)) {
                uint8_t tokenIndex = dtknqr & BITS(4, 0);
                uint8_t epIndex    = 0;
                if (tokenIndex < 6) {
                    epIndex = (dtknqr >> (4 * (tokenIndex + 1))) & 0xF;
                }
                else {
                    epIndex = (usb->DREGS.DTKNQR2 >> (4 * (tokenIndex - 5))) & 0xF;
                }
                if (self->IEPPendingMask & BIT(epIndex)) {
                    EndpointProcessWrite(self, epIndex);
                    self->Zoffy.IsTxFifoBusy  = true;
                    self->Zoffy.IEPProcessing = epIndex;
                }
            }
        }

        EndpointProcessWrite(self, self->Zoffy.IEPProcessing);
    }
    ...
    if (gintsts & USB_GINTSTS_IEPINT) { // [I] endpoint interrupt [R]
        ...
        while (epIntMask) {
            if (epIntMask & 1) {
                ...
                if (inEpInt & USB_DIEPINT_DISABLED) {
                    usb->IEPREGS[epIndex].DIEPINT = USB_DIEPINT_DISABLED;
                ...
                if (IsDedicatedTFifo() && inEpInt & USB_DIEPINT_FIFOEMPTY) { // TXFIFO empty
                    EndpointProcessWrite(self, epIndex);
                }
            }
            epIntMask >>= 1;
...
inline bool USBCoreZoffyConstractor(USBHALStruct* self, uint32_t usbBase) {
    memset(self, 0, sizeof(USBHALStruct));
    ...
    self->PhyType     = USBPhyTypeFS;
    self->SpeedConfig = USBSpeedConfigFull;

    self->USBBase = usbBase;

    USBTargetConstractor(self);
