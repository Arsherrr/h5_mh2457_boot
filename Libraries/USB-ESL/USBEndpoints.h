...
struct EPStruct_s {
    ...
    uint16_t  MaxPacket : 11; // Endpoint Real Max Packet
    bool      IsSetup : 1;    // Is setup transfer
...
// EPConfigStruct: 24 bytes
struct EPConfigStruct_s {
    uint8_t   Index : 4;            // Endpoint Index
    EPDirect  Direct : 4;           // Endpoint Direction
    EPType    Type : 2;             // Endpoint Type : Control / Interrupt / Bulk / ISO
    uint8_t   Timeout : 6;          // Endpoint Transfer Timeout (Tick), EPDirectOut only
    uint8_t   Interval;             // Endpoint Low/Full-Speed Interval
    uint8_t   HSInterval;           // Endpoint High-Speed Interval
    uint16_t  MaxPacket : 11;       // Endpoint Max Packet Size
    uint16_t  AdditionalPacket : 2; // Endpoint additional transaction opportunities per microframe for high-speed isochronous and interrupt endpoints
    uint16_t  AdditionalBuffer : 3; // Endpoint additional buffer counts
    uint16_t  MaxBuffer;            // Endpoint Max Single Transfer Size
    uint32_t* Buffer;
