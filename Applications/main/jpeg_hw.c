...
static void dma2d_yuv_to_rgb_init(const JPEG_InfoTypeDef *jpeg_info, uint32_t fmt)
{
    uint32_t align_base;
    uint32_t pl;
    uint32_t lo;
...
{
    return jpeg_hw_dec_handler(src, size, width, height, dst, dst_size, DMA2D_OUTPUT_MODE_RGB565);
}
 ...
 */
bool jpeg_hw_dec_handler(const uint8_t *src, uint32_t size, uint32_t width, uint32_t height, uint8_t *dst, uint32_t dst_size, uint32_t fmt)
{
    ...
    if (!wait_dma_tc(DMA2_Stream5, 100)) {
        JPEG_HW_LOG_ERROR("input dma timeout [ %X, %X, %X, %X, %X, %X, %X, %X ]\n", 
                jpeg_ram[0], jpeg_ram[1], jpeg_ram[2], jpeg_ram[3], 
                jpeg_ram[4], jpeg_ram[5], jpeg_ram[6], jpeg_ram[7]);
        jpeg_dma_stop();
        return false;
    }
    
    if (!wait_dma_tc(DMA2_Stream0, 100)) {
    ...
    dma2d_yuv_to_rgb_init(&jpeg_info, fmt);
