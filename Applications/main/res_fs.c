...
const jl_res_bin_header_t *g_res_hdr = (const jl_res_bin_header_t *)RES_BIN_BASE_ADDR;
const uint8_t *g_res_base = (const uint8_t *)RES_BIN_BASE_ADDR;
...
const jl_resource_info_t *find_res_by_path(const char *path)
{
    ...
    int index = perfect_hash(filename);
    if (index != -1) {
        /* 再次验证. */
        if (strcmp(filename, g_jl_resources[index].source_path + RES_BIN_BASE_ADDR) == 0)
        {
    ...
    // rt_kprintf("[RESFS] normalize: raw=%s rel=%s\n", path, match_path);
    ...
    for(uint32_t i = 0; i < JL_RESOURCE_COUNT; i++) {
        ...
        if(strcmp(sp_full, match_path) == 0 || strcmp(sp_base, rel_base) == 0 || strcmp(base_name(sp_full), base_name(match_path)) == 0) {
            // rt_kprintf("[RESFS] match: %s -> id=%u fmt=%u src=%s\n",
    ...
    rt_kprintf("[RESFS] match failed: %s base=%s\n", match_path, rel_base);
    return NULL;
#endif
}
...
static void * res_open_cb(lv_fs_drv_t * drv, const char * path, lv_fs_mode_t mode)
{
    ...
    // rt_kprintf("[RESFS] open hit: %s -> id=%u path=%s offset=%u size=%u type=%u\n",
    //            path, res->id, res->source_path, res->offset, res->size, res->format);
    const uint8_t *src = (const uint8_t *)(g_res_base + g_res_hdr->data_offset + res->offset);
    // rt_kprintf("[RESFS] head: %02X %02X %02X %02X %02X %02X %02X %02X\n",
...
static lv_fs_res_t res_read_cb(lv_fs_drv_t * drv, void * file_p, void * buf, uint32_t btr, uint32_t * br)
{
    ...
    if(f->pos >= res->size) {
        if(br) *br = 0;
        // rt_kprintf("[RESFS] read eof: pos=%u size=%u\n", f->pos, res->size);
        return LV_FS_RES_OK;
    }
    ...
    uint8_t *src = (uint8_t *)(g_res_base + g_res_hdr->data_offset + res->offset + f->pos);
    memcpy(buf, src, remain);
    f->pos += remain;
    if(br) *br = remain;
    if(remain > 0) {
        ;
        // rt_kprintf("[RESFS] read chunk: pos=%u remain=%u first=%02X %02X %02X %02X\n",
...
{
    printf("[RESFS] init check: base=0x%08X magic=0x%08X version=%u count=%u table_offset=%u hash_offset=%u data_offset=%u\n",
               (unsigned int)RES_BIN_BASE_ADDR,
               (unsigned int)g_res_hdr->magic,
               (unsigned int)g_res_hdr->version,
               (unsigned int)g_res_hdr->count,
               (unsigned int)g_res_hdr->table_offset,
               (unsigned int)g_res_hdr->hash_offset,
               (unsigned int)g_res_hdr->data_offset);

    if (g_res_hdr->magic != RES_MAGIC) {
