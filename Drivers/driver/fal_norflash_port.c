...
#define FLASH_SEC_SIZE         FLASH_SECTOR_SIZE
#define FLASH_START_ADDR       FLASH_ADDR_BASE
#define FLASH_END_ADDR         FLASH_ADDR_BASE + FLASH_SIZE  // 16*1024*1024
...
#define FAL_MEMCPY rt_memcpy

/**
 * @brief 需要实现以下函数
 */
static int norflash_init( void )
{
    return 0;
}
...
{
    if ((pbuf == RT_NULL) || (datalen == 0U))
    {
        return;
    }

    rt_memcpy(pbuf, (const void *)(addr), datalen);
}
...
static void norflash_write_page( uint8_t* pbuf, uint32_t addr, uint16_t datalen )
{
    ...
    flash_write_buf(pbuf, addr, datalen);
}

static void norflash_erase_sector( uint32_t saddr )
{
    /* norflash_write_page 自动擦除. */
    RT_UNUSED(saddr);
}
...
static int write_sector( long offset, const uint8_t* buf, size_t size )
{
    ...
    while ( cur_addr < addr_end_up ) {
        if ( cur_addr < addr ) {
            ...
            norflash_write_page( (uint8_t *)buf, addr, write_len );
            buf += write_len;
        }
            ...
            norflash_write_page( (uint8_t *)buf, cur_addr, write_len );
        }
        else {
            norflash_write_page( (uint8_t *)buf, cur_addr, FLASH_PAGE_SIZE );
            buf += FLASH_PAGE_SIZE;
        }
...
{
    uint32_t addr      = FLASH_START_ADDR + offset;
    uint32_t addr_up   = FAL_ALIGN_UP( addr, FLASH_SEC_SIZE );
    uint32_t addr_down = FAL_ALIGN_DOWN( addr, FLASH_SEC_SIZE );

    uint32_t addr_end      = addr + size;
    uint32_t addr_end_up   = FAL_ALIGN_UP( addr_end, FLASH_SEC_SIZE );
    uint32_t addr_end_down = FAL_ALIGN_DOWN( addr_end, FLASH_SEC_SIZE );
    uint32_t cur_addr      = addr_down;
    ...
    uint8_t* read_sector_buf = FAL_MALLOC( FLASH_SEC_SIZE );
    ...
    while ( cur_addr < addr_end_up ) {
        // 首次扇区写
        if ( cur_addr < addr ) {
            read( cur_addr - FLASH_START_ADDR, read_sector_buf, FLASH_SEC_SIZE );
            max_write_len = ( addr_up - addr );
            write_len     = size >= max_write_len ? max_write_len : size;
            if ( judge_whether_erase( read_sector_buf + addr - cur_addr, write_len ) ){
                ...
                write_sector( cur_addr, read_sector_buf, FLASH_SEC_SIZE );
            }
            else {
        ...
        //中间扇区写 直接擦除
            ...
            norflash_erase_sector( get_sector( cur_addr ) );
            write_sector( cur_addr, buf, FLASH_SEC_SIZE );
            buf += FLASH_SEC_SIZE;
        }
        cur_addr += FLASH_SEC_SIZE;
    }
